...
var Controllers = {
    I2CBACKPACK: {
        ...
        strip_color: {
            value: function(color, brightness = 0xff) {
                var strip = strips.get(this);
                var data   = [];
                ...
                data.push(brightness & FIRMATA_7BIT_MASK);
                data.push((brightness >> 7) & FIRMATA_7BIT_MASK);
                try {
...
Strip.prototype.colour = Strip.prototype.color = function(color, opts = { brightness: 0xff } ) {
    ...
    var stripcolor = null;
    var brightness;
    if(opts.brightness !== undefined) { brightness = opts.brightness; }
    ...
    if (stripcolor != null) {
        ...
        this.strip_color(ColorString.colorValue(stripcolor, strip.gtable), brightness);
...
Pixel.prototype.colour = Pixel.prototype.color = function(color, opts) {
    ...
    // strip.pixel(0).color('red', {brightness: 100})
    // strip.color('white', {brightness: 100});strip.show()
    // strip.show();
    var pixel = pixels.get(this);
    ...
    if (options.brightness != undefined) { brightness = opts.brightness; }
    var pixelcolor = null;
    ...
    if (pixelcolor != null) {
        ...
        if (sendmsg) {
            ...
            this.pixel_color(color, brightness);
        }
    } else {
